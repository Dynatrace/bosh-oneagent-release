...
docker run --rm --env "DEPLOYMENT_MOCK_URL=http://localhost:8080" --network container:dynatrace-deployment-api-mock $TAG_UBUNTU_1404 || fail

echo Running tests for Ubuntu 16.04...
docker run --rm --env "DEPLOYMENT_MOCK_URL=http://localhost:8080" --network container:dynatrace-deployment-api-mock $TAG_UBUNTU_1604 || fail
